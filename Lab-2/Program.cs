...

{
    ...
    abstract class Figure : IComparable, IPrint
    {
        ...
        public string Type  //E:\ВУЗ\СЕМ-3\БКИТ\LAB-2\LAB-2\App.config
        {
    ...
    class Square : Rectangle, IPrint
    {
        ...
        public override string ToString() //переопределяем для квадрата
        {
            return this.Type + " площадью " + this.Area().ToString() + " и стороной " + this.height;
        }
        public override void Print() //переопределяем для квадрата
        {
            Console.WriteLine(this.ToString());
        }
    }
    ...
    class Circle : Figure, IPrint
    {
       /// <summary>
        ...
        public override double Area() //переопределяем для круга
        {
            double Result = Math.PI * this.radius * this.radius;
            return Result;
        }
        public override string ToString() //переопределяем для круга
        {
            return this.Type + " площадью " + Math.Round(this.Area(), 3).ToString() + " и радиусом " + this.radius;
        }
        public override void Print() //переопределяем для круга
        {
            Console.WriteLine(this.ToString());
        }
    }
    ...
    internal class Program
    {
        ...
        {
            Figure figure1 = new Circle(6);

            Figure figure2 = new Rectangle(8, 7);
